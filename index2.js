...
async function fetchImages(access_token) {
    ...
    console.log("Total images:" + imageparams_list.length);
    console.log(imageparams_list);
    for(var i=0; i < imageparams_list.length; i++) {
        fetchImage(imageparams_list[i]);
    }

}

async function fetchImage(imageparam, access_token) {
    let image_download_endpoint = Object.values(imageparam)[0];
    let image_download_name = Object.values(imageparam)[1];
    ...
    try {
        ...
        if(resp.status = 200) {
            ...
            onsole.log("Error in loading image:" + image_download_name);
        }
    } catch(err) {
        console.log(err);
    }
...
async function displayImage(blob) {
    ...
    imgDiv.innerHTML=`<p>Image</p><img src="${objectUrl}"/>`
